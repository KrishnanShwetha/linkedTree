...
public class LinkedTree {
    ...
    /* contains the inner class for a postorder iterator
     * 
     */
    ...
    private class PostorderIterator implements LinkedTreeIterator {
    	...
    	private PostorderIterator() {
    	 ...
    	 public int next() {
             ...
             // Store a copy of the key to be returned.
             int key = nextNode.key;
             // Traverse to next nextNode
             Node trav = nextNode.parent;
             if (trav != null) {
                 if (trav.right != null && trav.right != nextNode) {
                     trav = trav.right;
                     trav = helper(trav);
                 }
             }
    	 ...
    	 private Node helper(Node trav) {
			 if (trav.left != null) {
				 return helper(trav.left);	 
			 }
			 else if (trav.right != null) {
				 return helper(trav.right);
			 }
			 return trav;
    ...
    System.out.println("(0) Testing PostorderIterator on {4, 10, 12, 15, 18, 22, 24, 25, 31, 35, 44, 50, 66, 70, 90}");
    try {
   	LinkedTree tree = new LinkedTree();
   	int[] keys = {4, 10, 12, 15, 18, 22, 24, 25, 31, 35, 44, 50, 66, 70, 90};
   	// expected output= {90, 70, 66, 50, 44, 35, 31, 25, 24, 22, 18, 15, 12, 10};
   	...
   	while (iter.hasNext()) {
        ...
        System.out.println("(1) Testing PostorderIterator on {59, 57, 55, 45, 69, 58, 56");
    try {
   	LinkedTree tree = new LinkedTree();
   	int[] keys = {59, 57, 55, 45, 69, 58, 56};
   	// expected output = {69, 59, 58, 57, 56, 55, 45};
   	tree.insertKeys(keys);
   	LinkedTreeIterator iter = tree.postorderIterator();
   	while (iter.hasNext()) {
   	   int key = iter.next();
